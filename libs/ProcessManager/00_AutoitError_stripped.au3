...
$iWidth += $aiColWidth[$i]
EndIf
If $i < 20 And $bDebug Then ConsoleWrite('@@ Debug(' & "arraydisplayinternals:445/1416" & ') : $iWidth = ' & $iWidth & " $i = " & $i & @CRLF)
Next
EndIf
If $iWidth > @DesktopWidth - 100 Then
...
Global $iWMICleanupInterval = 300000
Global $sWMIInstanceID = @YEAR & @MON & @MDAY & @HOUR & @MIN & @SEC & @MSEC
...
Local $hFile = FileOpen($sWatchdogLogPath, 1)
If $hFile <> -1 Then
...
Local $hFile = FileOpen($sWatchdogLogPath, 2)
If $hFile <> -1 Then
...
If $aWMIProcesses[0][0] > $iWarningWMI Then
...
Local $iCheckInterval = Int(IniRead($sWatchdogIniPath, "WMICleanup", "PeriodicCheckInterval", "600000"))
Local $iMaxWMI = Int(IniRead($sWatchdogIniPath, "WMICleanup", "MaxWMIProcesses", "8"))
Local $iWarningWMI = Int(IniRead($sWatchdogIniPath, "WMICleanup", "WarningWMIProcesses", "5"))
If TimerDiff($wmi_cleanup_timer) < $iCheckInterval Then Return
Local $iWMICount = WMI_CountProcessesLightweight()
If $iWMICount > $iMaxWMI Then
WriteWatchdogLog("ОШИБКА", "WMI_CLEANUP", "🚨 Критическое количество WMI процессов: " & $iWMICount & " (макс: " & $iMaxWMI & ")")
WMI_ForceReinitialize()
ElseIf $iWMICount > $iWarningWMI Then
...
Local $sFileName = StringRegExpReplace($sFullPath, ".*\\", "")
Return $sFileName & "|FULLPATH:" & $sFullPath
EndIf
...
Local $sExeDir = StringRegExpReplace($sFullPath, "\\[^\\]*$", "")
...
$bWMIEnabled = False
MsgBox(48, "⚠️ WMI Недоступен", "WMI служба недоступна!" & @CRLF & @CRLF & "🔧 Для тестирования отказа WMI:" & @CRLF & "1️⃣ Откройте services.msc" & @CRLF & "2️⃣ Найдите 'Windows Management Instrumentation'" & @CRLF & "3️⃣ Остановите службу" & @CRLF & @CRLF & "🔄 Система продолжит работу через ProcessExists")
EndIf
EndFunc
...
If ($sErrorNumber = "80020009" And ( StringInStr($sErrorDescription, "time") > 0 Or StringInStr($sErrorDescription, "время") > 0 Or StringInStr($sErrorDescription, "ожидани") > 0 Or StringInStr($sErrorDescription, "Timed out") > 0 Or StringInStr($sErrorDescription, "Прекращено") > 0)) Or ($sErrorNumber = "80020006") Then
SetError(0)
Return
EndIf
$iWMIErrors += 1
...
If StringInStr($sErrorDescription, "Invalid class") > 0 Or StringInStr($sErrorDescription, "Provider failure") > 0 Or StringInStr($sErrorDescription, "Access denied") > 0 Or StringInStr($sErrorDescription, "RPC server") > 0 Or StringInStr($sErrorDescription, "RPC Server") > 0 Then
$bCriticalError = True
...
Local $sAutoitLogEntry = "--- CRASH REPORT ---" & @CRLF & "Time: " & @YEAR & "/" & @MON & "/" & @MDAY & " " & @HOUR & ":" & @MIN & ":" & @SEC & @CRLF & "Process: " & $sAppName & @CRLF & "Line in EXE: " & $sLineNum & @CRLF & "Variable: " & $sVarName & @CRLF & "Description: " & $sErrorDesc & @CRLF & "Full Raw: " & StringStripWS($sRawText, 3) & @CRLF & "-----------------------" & @CRLF
